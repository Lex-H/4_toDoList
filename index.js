...
//         // 儲存變更到proxyListDataData
//         var index = event.target.parentElement.getAttribute("id");
//         proxyListDataData[index].title = toDoSave;
...
function liEvent() {
  ...
  list.addEventListener('click', function(ev) {
    ...
    if (classClicked.includes('toDo')) {
      ...
      var newInput = document.getElementsByClassName("inputToDoContent")[0];
      var toDoSave = newInput.value; // 使用 value 屬性
      ...
      newInput.addEventListener('input', function(event) {
        ...
        // 自動儲存內容到變數 toDoSave
        toDoSave = event.target.value; // 使用 value 屬性
      });

      // 事件：監聽失去焦點，恢復原本內容
      newInput.addEventListener('focusout', function(event) {
        if (event.target.parentNode) { // 確認元素是否有父節點
          // 儲存變更到proxyListDataData
          var index = event.target.parentElement.getAttribute("id");
          proxyListDataData[index].title = toDoSave;
          ...
          // 恢復原本內容
          var toDo = `<div class="toDo">${toDoSave}</div>`;
          event.target.outerHTML = toDo;
        }
      });
    }
    ...
    // 點擊事件：delete
    if (classClicked.includes('delete')) {
      var index = ev.target.parentElement.getAttribute("id");
      proxyListDataData.splice(index, 1);
    }
  }, false);
}
...
// 函數：綁定所有toDoListUL裡面的事件
function toDoListULEvent() {
  liEvent();
  addToDoEvet();
}